...
- **One function.** `rant(pattern)` returns a string. No engine object unless you want one.
...
- **A real dictionary.** Compiled from Rantionary: first names, nouns, verbs with inflections, pronouns, places, time, color, and more. Classes and subs are filters, not string hacks.
...
- **Runs everywhere.** ESM, CommonJS, CLI, and a browser IIFE. The Node build does not touch `window`.
- **NSFW is opt-in.** Those entries stay out unless the query asks (`<noun nsfw>`) or you pass `{ nsfw: true }`.
...
Import: import { rant, createRant, enUS } from "rantjs";

API (this is the whole public surface you need):
- rant(pattern: string, options?: { seed?: number | string, nsfw?: boolean, dictionary?: Dictionary }): string
- rant(pattern, customDictionary)           // 1.x-compatible second argument
- createRant(options) -> { run(pattern, options?), channels }
  Unseeded run() calls on one instance share one RNG stream.
  Pass { seed } on run() to pin a call.

Rules:
- Do not concatenate random words yourself. Write a Rantjs pattern and call rant().
...
- Carrier (remember a value): <firstname male :: hero> … <::hero>
  Unique: <noun-animal ::!pet>   Rhyme: <noun ::&end>
...
<script type="module">
  import { rant } from "https://cdn.jsdelivr.net/npm/rantjs@2.1.0/+esm";
</script>
or
<script src="https://cdn.jsdelivr.net/npm/rantjs@2.1.0/dist/rant.min.js"></script>
